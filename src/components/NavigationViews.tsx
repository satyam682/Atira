...
export function AdminPanelView({ showToast }: ViewProps) {
  const [activeTab, setActiveTab] = useState<'users' | 'invitations' | 'add-user' | 'pricing' | 'api-config'>('invitations');
  ...
  return (
    <div className="px-4 py-6 md:p-8 space-y-8 max-w-6xl mx-auto font-sans bg-canvas text-body animate-in fade-in duration-300 w-full overflow-x-hidden">
      ...
      <div className="border-b border-hairline flex items-center gap-2 overflow-x-auto pb-0.5">
        ...
        >
          <Sliders className="w-3.5 h-3.5" /> API Configuration
        </button>
      </div>
      ...
      {activeTab === 'api-config' && (
        <div className="space-y-6 animate-in fade-in duration-200">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            {/* LEFT COLUMN: FORM */}
            <div className="lg:col-span-5 bg-surface-card border border-hairline p-6 rounded-xl shadow-none space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="font-medium text-ink flex items-center gap-2 font-serif">
                  <Sliders className="w-4 h-4 text-primary" /> {formConfig.id ? 'Edit Upstream Node' : 'Configure Upstream Node'}
                </h3>
                {formConfig.id && (
                  <button
                    type="button"
                    ...
                    className="text-[10px] text-muted hover:text-ink underline transition-colors cursor-pointer"
                  >
                    Cancel Edit
                  </button>
                )}
              ...
              <p className="text-xs text-muted-soft leading-relaxed">
                Connect and prioritize a model provider endpoint. When the primary key hits a rate limit, the router dynamically cascades down to the next active node.
              </p>

              <form onSubmit={handleSaveConfig} className="space-y-3 pt-2">
                <div>
                  <label className="text-[10px] font-semibold text-muted uppercase tracking-wider block mb-1">Friendly Label</label>
                  <input
                    type="text"
                    required
                    placeholder="e.g., Primary Cohere Key, Backup Provider"
                    value={formConfig.label}
                    onChange={(e) => setFormConfig({ ...formConfig, label: e.target.value })}
                    className="w-full px-3 py-1.5 bg-canvas border border-hairline rounded-lg text-xs text-body focus:outline-none focus:border-primary transition-colors"
                  />
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] font-semibold text-muted uppercase tracking-wider block mb-1">Provider Core</label>
                    <select
                      value={formConfig.provider}
                      onChange={(e) => setFormConfig({ ...formConfig, provider: e.target.value })}
                      className="w-full px-3 py-1.5 bg-canvas border border-hairline rounded-lg text-xs text-body focus:outline-none focus:border-primary transition-colors cursor-pointer"
                    >
                  ...
                  <div>
                    <label className="text-[10px] font-semibold text-muted uppercase tracking-wider block mb-1">Priority Order</label>
                    <input
                      type="number"
                      min="1"
                ...
                <div>
                  <label className="text-[10px] font-semibold text-muted uppercase tracking-wider block mb-1">API Endpoint URL</label>
                  <input
                    type="url"
                    required
                    placeholder="https://api.cohere.com/v2/chat"
                    value={formConfig.endpoint_url}
                    onChange={(e) => setFormConfig({ ...formConfig, endpoint_url: e.target.value })}
                    className="w-full px-3 py-1.5 bg-canvas border border-hairline rounded-lg text-xs text-body focus:outline-none focus:border-primary transition-colors font-mono"
                  />
                </div>
                ...
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <label className="text-[10px] font-semibold text-muted uppercase tracking-wider block">API Access Token / Secret Key</label>
                    <button
                      type="button"
                      onClick={() => setShowApiKey(!showApiKey)}
                      className="text-[9px] text-primary hover:underline font-medium cursor-pointer text-right"
                    >
                  ...
                  <input
                    ...
                    required
                    placeholder={formConfig.id ? '••••••••••••' : 'Paste API Key token here'}
                ...
                <div>
                  <label className="text-[10px] font-semibold text-muted uppercase tracking-wider block mb-1">Initial Node Status</label>
                  <select
                    value={formConfig.status}
                    onChange={(e) => setFormConfig({ ...formConfig, status: e.target.value })}
                    className="w-full px-3 py-1.5 bg-canvas border border-hairline rounded-lg text-xs text-body focus:outline-none focus:border-primary transition-colors cursor-pointer"
                  >
                    <option value="active">Active (Routable)</option>
                    <option value="standby">Standby (Warm Redundancy)</option>
                    <option value="exhausted">Exhausted (Rate Limited)</option>
                    <option value="invalid">Invalid (Expired Credentials)</option>
                  </select>
                </div>

                {formTestResult && (
                  <div className={`p-3 rounded-lg text-xs ${formTestResult.success ? 'bg-emerald-50 text-emerald-800 border border-emerald-100' : 'bg-rose-50 text-rose-800 border border-rose-100'}`}>
                    <p className="font-semibold">{formTestResult.success ? '✓ Verification Successful' : '✗ Verification Failed'}</p>
                    <p className="mt-0.5 text-[11px] leading-relaxed font-mono truncate">{formTestResult.message}</p>
                  </div>
                )}
                ...
                <div className="flex gap-2 pt-2">
                  <button
                    ...
                    className="flex-1 py-2 border border-hairline hover:bg-surface-soft text-muted rounded-lg text-xs font-semibold transition-all flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
                  >
                    {formTesting ? (
                      <>
                      ...
                      <>
                        <Play className="w-3.5 h-3.5" /> Test Configuration
                      </>
                    )}
                  </button>
                  <button
                    type="submit"
                    className="flex-1 py-2 bg-primary hover:bg-primary-active text-white rounded-lg text-xs font-semibold shadow-none transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                    <Check className="w-3.5 h-3.5" /> {formConfig.id ? 'Save Changes' : 'Register Node'}
                  </button>
                </div>
              </form>
            </div>
            ...
            <div className="lg:col-span-7 space-y-6">
              <div className="bg-surface-card border border-hairline rounded-xl overflow-hidden shadow-none">
                <div className="p-5 border-b border-hairline-soft flex items-center justify-between">
                  <div>
                    <h3 className="font-medium text-ink text-sm font-serif">Dynamic Routing Registry</h3>
                    <p className="text-xs text-muted-soft mt-0.5">List of all active, standby, and exhausted upstream model providers.</p>
                  </div>
                  <button
                    type="button"
                    onClick={fetchConfigs}
                    className="p-1.5 border border-hairline rounded-lg hover:bg-surface-soft text-muted hover:text-ink transition-colors cursor-pointer"
                    title="Refresh List"
                  >
                ...
                {configsLoading && configs.length === 0 ? (
                  <div className="p-12 text-center text-xs text-muted-soft">Loading registry entries...</div>
                ) : configs.length === 0 ? (
                  <div className="p-12 text-center text-xs text-muted-soft italic">No configured upstream nodes. Add one on the left to start!</div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-xs border-collapse">
                      <thead>
                        <tr className="bg-surface-soft text-muted uppercase tracking-wider text-[10px] font-medium border-b border-hairline-soft">
                          <th className="p-4">Friendly Name</th>
                          <th className="p-4">Model & Key</th>
                          <th className="p-4">Priority / Limit</th>
                      ...
                      <tbody className="divide-y divide-hairline-soft font-sans">
                        {configs.map((cfg) => {
                          const isActivePriority = cfg.priority === 1 && cfg.status === 'active';
                          return (
                            <tr key={cfg.id} className={`hover:bg-surface-soft/30 transition-colors ${isActivePriority ? 'bg-primary/[0.02]' : ''}`}>
                              <td className="p-4 font-medium text-ink">
                                <div className="space-y-0.5">
                                  <div className="flex items-center gap-1.5">
                                    <span className="truncate">{cfg.label}</span>
                                    {isActivePriority && (
                                      <span className="text-[9px] bg-emerald-100 text-emerald-800 font-bold px-1.5 py-0.2 rounded">PRIMARY</span>
                                    )}
                                  </div>
                                  <div className="text-[10px] text-muted-soft font-mono truncate max-w-xs">{cfg.endpoint_url}</div>
                                </div>
                              </td>
                              <td className="p-4">
                                <div className="space-y-0.5">
                                  <div className="font-mono text-xs">{cfg.model_name}</div>
                                  <div className="font-mono text-[10px] text-muted-soft">{cfg.api_key}</div>
                                </div>
                              </td>
                              <td className="p-4">
                                ...
                                <div className="flex items-center justify-end gap-1.5">
                                  {cfg.status !== 'active' && (
                                    <button
                                      ...
                                      className="px-2 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded text-[10px] font-bold shadow-sm cursor-pointer transition-all"
                                      title="Set Active & Prioritize"
                                    >
                                      Set Active
                                    </button>
                                  )}
                                  <button
                                    ...
                                    className="p-1 border border-hairline hover:bg-surface-soft rounded text-muted-soft hover:text-ink cursor-pointer transition-all"
                                    title="Edit Config"
                                  >
                                  ...
                                  <button
                                    ...
                                    onClick={() => handleTestConfigInline(cfg)}
                                    className="p-1 border border-hairline hover:bg-surface-soft rounded text-muted-soft hover:text-ink cursor-pointer transition-all"
                                  ...
                                  <button
                                    ...
                                    className="p-1 border border-hairline hover:bg-rose-50 rounded text-muted-soft hover:text-rose-600 cursor-pointer transition-all"
                                    title="Delete Node"
                                  >
              ...
              {testResult && (
                <div className={`p-4 rounded-xl text-xs space-y-1 animate-in slide-in-from-top-2 duration-200 border ${
                  testResult.success ? 'bg-emerald-50 text-emerald-800 border-emerald-100' : 'bg-rose-50 text-rose-800 border-rose-100'
                }`}>
                  <div className="flex justify-between items-center">
                    <span className="font-semibold">{testResult.success ? '✓ Integration Test Succeeded' : '✗ Integration Test Failed'}</span>
                    <button type="button" onClick={() => setTestResult(null)} className="text-[10px] opacity-70 hover:opacity-100">Dismiss</button>
                  </div>
                  <p className="text-[11px] font-mono leading-relaxed">{testResult.message}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-surface-card border border-hairline p-6 rounded-xl shadow-none space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="font-medium text-ink flex items-center gap-2 font-serif">
                <Clock className="w-4 h-4 text-primary" /> System Audit Trail & Failover Log
              </h3>
              <button
                type="button"
            ...
            <p className="text-xs text-muted-soft leading-relaxed">
              Cryptographically verified audit trail documenting admin configuration updates, dynamic routing, fallback occurrences, and system validations.
            </p>
            <div className="space-y-2 max-h-64 overflow-y-auto pt-2 font-mono text-[10px] leading-relaxed text-body bg-canvas/30 p-3 rounded-lg border border-hairline">
              {logsLoading && auditLogs.length === 0 ? (
                <p className="text-muted-soft italic text-center py-4">Reading system logs...</p>
              ) : auditLogs.length === 0 ? (
                <p className="text-muted-soft italic text-center py-4">No logged occurrences in system history.</p>
              ) : (
                auditLogs.map((log, index) => (
                  <div key={index} className="border-b border-hairline-soft/40 pb-2 last:border-b-0">
                    <p className="text-primary font-semibold flex items-center gap-1.5">
                      <span>[{log.action.toUpperCase()}]</span>
                      <span className="text-muted-soft font-normal text-[9px]">{new Date(log.timestamp).toLocaleString()}</span>
                    </p>
                    <p className="text-body mt-0.5">{log.details}</p>
                    <p className="text-[9px] text-muted-soft mt-0.2">Initiated by: {log.admin_email}</p>
                  </div>
                ))
              )}
            </div>
